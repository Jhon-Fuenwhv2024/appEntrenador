...
- Móvil + bottom nav: no tapar contenido; clearance vía `--tf-bottom-nav-clearance` en `.shell-body` ([`.cursor/rules/ui-bottom-nav-clearance.mdc`](.cursor/rules/ui-bottom-nav-clearance.mdc)).
