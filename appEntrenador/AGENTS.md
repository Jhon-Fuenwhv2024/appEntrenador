...
- Móvil + bottom nav: nav `fixed` + clearance en el scroller vía `--tf-bottom-nav-clearance` ([`.cursor/rules/ui-bottom-nav-clearance.mdc`](.cursor/rules/ui-bottom-nav-clearance.mdc)).
