...
- `src/shared/api/http.js`: instancia Axios única (`baseURL` desde `resolveApiBaseUrl()`), Bearer JWT, refresh silencioso en 401 (Feature 083) e interceptor de errores.
- `src/shared/auth/session.js`: persistencia de token + datos de usuario en `localStorage`.
- `src/shared/layout/AppShell.vue` + `AppBottomNav.vue`: shell autenticado (sidebar desktop / bottom nav móvil ≤960px). Estilos en `src/assets/appShell.css`. Bottom nav `position: fixed`; clearance vía `--tf-bottom-nav-clearance` en `.shell-body` + `.main-content`. Escala tipográfica in-app por `font-size` en `html` (no CSS `zoom`). Al cambiar de ruta, `scrollToTop.js` lleva el scroll al tope.
  - **Trainer (4 slots):** Inicio (`/dashboard`), Alumnos (`/trainer/clients`), Recursos (`/trainer/library`), Ajustes (`/trainer/settings`). Logout: menú de cuenta en header (Feature 063) + pie sidebar desktop.
  ...
  - **Contexto (no ítem de barra):** ficha `/trainer/clients/:clientId` marca Alumnos; catálogo `/trainer/exercises` es herramienta de Recursos (sin slot propio).
...
- `src/features/trainer/`: portal del entrenador, clientes, invitaciones (`InvitesManager` en Alumnos + `InviteClientAction` en Inicio), ficha 360 (`client-360/Client360View` en `/trainer/clients/:clientId`; Programación 061 = week board + day builder + assign plantilla), lista de alumnos (`ClientsListView` con avatar + eliminar), hub **Recursos** (`LibraryView` en `/trainer/library`; tabs plantillas/catálogo/membresías/dietas), placeholder `TrainerSettingsView` (024). Inicio (`TrainerDashboardView`) es hub de métricas + invitación + CTA a Alumnos y a **Recursos → Dietas** (`/trainer/library/diets`). Paywall 402 (Feature 037) en create-invite.
...
- Frontend trainer: `LibraryView` es el hub **Recursos** (tabs Plantillas | Catálogo | Membresías | Dietas en `/trainer/library*`); ficha 360 / `TemplateFormDialog` usan autocomplete híbrido (`nombre` + `exercise_id`).
